const MagicButton = ({ title, icon, position, handleClick, otherClasses }) => {
    return (
      <div
        ...
        onClick={handleClick}
      >
        <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_35%,#393BB2_50%,#E2CBFF_100%)]" />
        ...
        >
          ...
          <span className="text-center text-ellipsis whitespace-nowrap">{title}</span> {/* Ensures the title stays centered */}
        </span>
      </div>
    );
  };
